...
def draw(angles) -> None :
	...
	pitch *= 180.0 / math.pi
	yaw   *= 180.0 / math.pi
	yaw   -= -1.15  # Declination at Chennai, TamilNadu, India is - 1 degress 15 min
	roll  *= 180.0 / math.pi
...
def process_data(acc_path: str, gyro_path: str, mag_path: str, frame='NED') -> np.ndarray:
	# process data
	acc = pd.read_csv(f'./sensor_data/{acc_path}')
	gyro = pd.read_csv(f'./sensor_data/{gyro_path}')
	mag = pd.read_csv(f'./sensor_data/{mag_path}')
